...
if($button1)
{
  $sql="UPDATE attempt_book_tb SET Approved_Not_yet='$apprve' WHERE Record_id='" . $_POST['record'] . "'";
...
<?php
      $connect=mysqli_connect("localhost","root","","kcar-rental");
                ...
                $result = mysqli_query($connect,"SELECT * FROM attempt_book_tb WHERE Approved_Not_yet='Not yet approved'");
echo"<center>";

echo "<table cellspacing='0' cellpodding='0'  width='500' height='200' id='whtc' border='1' bgcolor='white'>
<tr>
<th id='row1'>Book_id</th>
<th id='row1'>Plate</th>
<th id='row1'>Image</th>
...
<th id='row1'>Contact</th>
<th id='row1'>Pick up date</th>
<th id='row1'>Drop off date</th>
...
while($row = mysqli_fetch_array($result))
  ...
  echo "<tr id='row1'>";
  echo "<td id='row0'>" . $row['Record_id'] . "</td>";
  echo "<td id='row0'>" . $row['Plate_number'] . "</td>";
  echo "<td id='row0'>";?> <img width='90' height='60' src=<?php echo "image/".$row['image_name'].">"?> <?php echo "</td>";
  echo "<td id='row0'>" . $row['mark_name'] . "</td>";
  echo "<td id='row0'>" . $row['Price'] . "</td>";
  echo "<td id='row0'>" . $row['F_name'] . "</td>";
  echo "<td id='row0'>" . $row['L_name'] . "</td>";
  echo "<td id='row0'>" . $row['Phone_number'] . "</td>";
  echo "<td id='row0'>" . $row['Pdate'] . "</td>";
  echo "<td id='row0'>" . $row['Rdate'] . "</td>";
...
echo"</center>";
?>
<form method="post" action="peaplebook.php">
  <center>
<select id="user" class="text" name="record">
                  <?php
                   ...
                   $result = mysqli_query($connect,"SELECT * FROM attempt_book_tb WHERE Approved_Not_yet='Not yet approved'");
                    ...
                    {
                    ?>
                  <option name="record"><?php echo $row["Record_id"];?></option>
                  <?php
                      }
                  ?>
                </select>
                
<input id="button" type="submit" value="Approve request" name="button2">
</form>
</center>
<br>
<br>
...
<?php
      $connect=mysqli_connect("localhost","root","","kcar-rental");
                ...
                $result = mysqli_query($connect,"SELECT * FROM attempt_book_tb WHERE Approved_Not_yet='Approved'");
echo"<center>";
